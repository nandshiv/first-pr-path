...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://first-pr-path.vercel.app",
        "http://localhost:5173",
    ],
...
def get_repo_files(repo_id: str, db: Session = Depends(get_db)):
    ...
    centrality_scores = {
        ...
        for s in db.query(FileCentrality)
    ...
    return [
        {
            ...
            "centrality_score": centrality_scores.get(f.file_path),
        }
        for f in files
    ]
...
def explain_file_endpoint(repo_id: str, file_path: str, db: Session = Depends(get_db)):
    return explain_file(db, repo_id, file_path)
...
def report_resolution(issue_id: str, pr_url: str = None, db: Session = Depends(get_db)):
    ...
    return {
        "status": "report recorded, will be verified against GitHub separately"
    }
...
def get_recommendations(repo_id: str, skill_profile: str, db: Session = Depends(get_db)):
    ...
    candidates = score_issues_for_user(
        db,
        repo_id,
        skill_profile,
        top_n=15,
    ...
    verified = get_verified_open_issues(
        db,
        repo,
        candidates,
        needed=5,
